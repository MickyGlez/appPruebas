...
public class TodosEjercicios {
	public static void main(String[] args) {
		...
		for (int numerosIndice = 0; numerosIndice < numEnteros.length; numerosIndice++) {
			numEnteros[numerosIndice] = (int) (Math.random() * 100);
			// Ordenar Numeros ASC
			Arrays.sort(numEnteros, 0, 100);
			System.out.println(numerosIndice +" " + numEnteros[numerosIndice]);
			...
			for (i = 0; i < numEnteros.length; i++) {
				...
				for (int sumX = 0; sumX < numEnteros.length; sumX++) {
					...
					sumaPU = numEnteros[0] + numEnteros[100];
				}
			}
		}
